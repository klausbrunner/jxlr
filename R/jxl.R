...
write_jxl_anim <- function(frames, target = NULL, durations = 100, 
  ...
  processed_frames <- lapply(frames, function(frame) {
    if (is.numeric(frame)) frame <- aperm(structure(as.raw(frame * 255), dim = dim(frame)))
  ...
  if (!is.na(quality)) stopifnot("quality must be between 0 and 100" = quality >= 0 && quality <= 100)
  if (!is.na(effort)) stopifnot("effort must be between 1 and 9" = effort >= 1 && effort <= 9)
  
  buf <- .Call("_jxlr_jxl_encode_anim_raw", processed_frames, durations, quality, effort, loop_count, PACKAGE = "jxlr")
  
  if (is.character(target)) { writeBin(buf, target); invisible(target) } else structure(buf, class = "jxl_anim")
}
...
#' Convert image arrays between different dimension conventions
#'
#' Helper functions to convert between R's typical array formats and JXL's expected format.
#'
#' @param img Image array
#' @return Converted image array
#' @examples
#' \donttest{
#' # Convert from channels-last (common in R) to channels-first (JXL)
#' img_hwc <- array(runif(10*8*3), dim = c(10, 8, 3))  # Height x Width x Channels
#' img_chw <- from_channels_last(img_hwc)               # Channels x Height x Width
#' dim(img_chw)  # c(3, 10, 8)
#' 
#' # Convert back
#' img_back <- to_channels_last(img_chw)
#' dim(img_back)  # c(10, 8, 3)
#' }
#' @export
from_channels_last <- function(img) {
  if (length(dim(img)) != 3) stop("Image must be a 3D array")
  aperm(img, c(2, 1, 3))  # H x W x C -> W x H x C
}

#' @rdname from_channels_last
#' @export
to_channels_last <- function(img) {
  if (length(dim(img)) != 3) stop("Image must be a 3D array")
  aperm(img, c(2, 1, 3))  # W x H x C -> H x W x C
}

#' @rdname from_channels_last
#' @export
from_raster <- function(img) {
  if (!inherits(img, c("RasterLayer", "RasterBrick", "RasterStack"))) {
    stop("Input must be a Raster object")
  }
  
  if (!requireNamespace("raster", quietly = TRUE)) {
    stop("raster package required for from_raster()")
  }
  
  # Convert raster to array
  arr <- raster::as.array(img)
  
  # Raster arrays are typically Y x X x Bands, but Y is flipped
  # Need to flip Y axis and convert to our expected format
  if (length(dim(arr)) == 3) {
    # Multi-band: flip Y and reorder to C x H x W
    arr <- arr[nrow(arr):1, , ]  # Flip Y axis
    aperm(arr, c(3, 1, 2))       # Bands x Y x X
  } else {
    # Single band: flip Y, add channel dimension, reorder
    arr <- arr[nrow(arr):1, ]    # Flip Y axis
    dim(arr) <- c(dim(arr), 1)   # Add channel dimension
    aperm(arr, c(3, 1, 2))       # 1 x Y x X
  }
}
